...
if (empty($_GET['filename']) || empty($_GET['host']) || empty($_GET['path'])) {
	...
	include(TEMPLATE_DIR . '/headerr.php');
	if ($options['ref_check']) check_referer();
	...
	do {
		...
		if ($_GET['auth'] == '1') {
			...
			} else html_error('No usable premium account found for this download');
	...
	elseif ($file['bytesReceived'] == $file['bytesTotal'] || $file['size'] == 'Unknown') {
		...
		else echo "<br /><a class='btn btn-primary' href='$PHP_SELF' target='_blank'>" . lang(13) . "</a> <br /><br /><br />";
