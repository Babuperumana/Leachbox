...
class youtube_com extends DownloadClass {
	...
	private function getCipher() {
		...
		$page = $this->GetPage('http://www.youtube.com/embed/'.$this->vid, $this->cookie);
	...
	private function QSelector() {
		...
		echo "\n<h3 style='text-align: center;'>".lang(216)."</h4>";
		echo "\n<center><form name='YT_QS' action='{$GLOBALS['PHP_SELF']}' method='POST'>\n";
		echo "<input type='hidden' name='yt_QS' value='on' />\n";
		echo '<div class="form-group">';
		echo '<div class="checkbox">';
		echo '<label><input type="checkbox" name="cleanname" checked="checked" /><small>&nbsp;Remove non-supported characters from filename</small></label>';
		echo '</div></div>';
		echo '<div class="form-group">';
		echo "<select class='form-control' name='yt_fmt' id='QS_fmt'>\n";
		...
		echo "<input class='btn btn-primary' type='submit' name='Th3-822' value='".lang(209)."' />\n";
		echo "</form></center>\n</body>\n</html>";
		exit;
	}
}
